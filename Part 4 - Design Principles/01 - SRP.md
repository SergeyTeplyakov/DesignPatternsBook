...
Основным строительным блоком объектно-ориентированного приложения является класс, поэтому обычно принцип единственной обязанности рассматривается в контексте класса. Но, поскольку основую работу выполняют методы, то очень важно, чтобы они также были нацелены на решение одной задачи. 
...
Развитие программного проекта приводит к увеличению хаоса. Спешка, непродуманные решения, не полной понимание намерений оригинального решения, все это приводит к постепенному ухуджению качества дизайна. При внесении изменений очень важно оценировать качество решения на предмет его чистоты, соответствия принципам проектирования и здравому смыслу. То, что на первых этапах должно было находится в одном классе, должно быть перемещено в совершенно другое место, а логика, которая помещалась в одном методе, может перерости в целую иерархию классов.
...
class LogEntryParser
{
    public bool TryParse(string line, out LogEntry logEntry)
    {
        ...
        // и для получения записи нужно проанилизировать еще одну или более строку
    }
}
...
Итеграционные тесты показывают корректность реализации в реальном окружении. 
