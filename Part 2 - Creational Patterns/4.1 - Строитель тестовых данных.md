...
В результате мы получаем лишнюю связанность (coupling) между методом инициализации и методами тестов, что приводит к постоянному "переключению контекста" при чтении кода. К тому же, мы не можем контролировать процесс создания тестируемого объекта и не можем задать нужные аргументы конструктора. Самый простой способ решения этих проблемы заключается в выделении специального фабричного метода, предназначенного для создания тестируемого класса. Называется такой паттерн "Родитель объектов" (Object Mother). 

(*) Сноска: когда я попытался перевести название этого паттерна на русский язык, то оказался в тупике. Какой вариант использовать "Матерь объектов", "Фабрика объектов", "Родитель объектов"? Первый вариант звучит ужасно, а второй не отражает исходной сути. Третий вариант - неплох Еще приходит на ум вариант "Родитель объектов". В результате, пришлось оставить оригинальный термин.

## Родитель объектов (Object Mother)
