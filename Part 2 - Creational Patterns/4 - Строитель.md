...
```
Листинг 4.11 – Использования строителя для создания неизменяемой коллекции

### Частичная изменяемость
Неизменяемые коллекции показывают хороший пример частичной неизменяемости. С точки зрения внешних клиентов объекты класса `ImmutableList<T>` являются неизменяемым. Но есть одно исключение: вложенный класс `ImmutableList<T>.Builder`, который может получить непосредственный доступ к закрытым полям класса `ImmutableList<T>`.
